...
class Textarea{
    ...
    private function field_markup() {
        ...
        echo '<div class="wsf-single-field wsf-textarea '.esc_attr( $attr['wrap_class'].esc_attr( $inline ) ).'">
        <label>'.esc_html( $attr['label'] ).'</label>
        <textarea name="'.$optName.'['.$name.']" class="'.esc_attr( $class ).'" rows="5" cols="10" placeholder="'.esc_attr( $attr['placeholder'] ).'" id="'.esc_attr( $id ).'" >'.esc_html( $value ).'</textarea></div>';
