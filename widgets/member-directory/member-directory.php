...
private function member_layout_style_controls()
{
					...
					<div class="cooalliance-loader" style="display:none">
      				<div class="cooalliance-loader-inner">
      					...
      					<p class="cooalliance-loading-text">Member Searching....</p>
								...
								if(!empty($query)):
								?>
				   			<div class="elementor-grid coo-member-wapper  <?php echo $columns_desktop.$columns_tablet.$columns_mobile ?>" id="search-filter-data">
							<?php
							foreach ($query as $member_info):
							$nick_name = get_user_meta( $member_info->ID, 'nickname', true);
							$first_name = get_user_meta( $member_info->ID, 'first_name', true);
							$last_name = get_user_meta( $member_info->ID, 'last_name', true);
							$company = get_user_meta( $member_info->ID, 'company_name', true);
							$user_image = get_user_meta( $member_info->ID, 'image', true);

							?>
							<div class="card cooalliance_member_list_card coo-member-card-wrapper">
							    ...
							    <?php echo get_avatar( $member_info->ID, 'medium', '', 'member-profile', array('class' => array('img-fluid', 'rounded-circle') )); ?>
								  ...
								  <div class="card-body">
										...
										<a href="<?php echo get_author_posts_url( $member_info->ID, $nick_name ); ?>"	>
												<h5 class="card-title">
                                                    <?php echo $member_info->user_firstname .' '. $member_info->user_lastname ; ?>
												</h5>
										</a>
			      	...
			      	<?php endforeach; ?>
			      	<?php  else:?>
				         	<h4><?php _e("No member found!", "collaliance"); ?></h4>
				     <?php endif; ?>
					 <br>
						 <?php
							...
							echo '<div id="pagination" class="clearfix coo-pagination-wrapper members-pagination">';
   ...
   <?php
	...
	public function member_search_form()
	{
        ...
        ?>
			<div class="coo-searchform-wrapper">
				<form id="MemberSearchForm" method="POST">
							<label><?php esc_html_e('Search By Name, City, Industry, etc','cooalliance')?></label>
